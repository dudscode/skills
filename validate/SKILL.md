...
2. **Ativação da Skill de Validação:** Se a tarefa for identificada como sensível, o agente ativa a Skill de Validação correspondente (por exemplo, `payment-compliance-validator` para transações financeiras ou `validador-identidade` para personalização de documentos).
